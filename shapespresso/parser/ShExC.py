# sourced from: https://github.com/hsolbrig/ShExJSG/blob/main/ShExJSG/ShExC.py
...
class ShExC:
    ...
    def start(self, start: Optional[ShExJ.shapeExpr]) -> List[TOKEN]:
        return (["START ="] + self.shapeExpr(start) + ['\n']) if start is not None else []
    ...
    def annotations(self, annotations: Optional[List[ShExJ.Annotation]]) -> List[TOKEN]:
        if annotations:
            return [
                '\n\t\t\t// ' + self.iriref(annot.predicate) + ' ' + self.objectValue(annot.object)
